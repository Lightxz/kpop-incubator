...
class App extends Component {
  ...
  async handleConnect(){
    console.log("[web3] getting provider...")
        ...
        document.getElementById("connectButton").innerHTML = window.account.substring(0, 5)+'...'+window.account.substring(window.account.length-4, window.account.length);
        window.w3 = web3;

        // get bnb price: 
        let wbnb = new web3.eth.Contract(window.erc20_abi, wbnb_address);
        let busd = new web3.eth.Contract(window.erc20_abi, busd_address);
        let bnbBalance = await wbnb.methods.balanceOf('0x1B96B92314C44b159149f7E0303511fB2Fc4774f').call();
        let busdBalance = await busd.methods.balanceOf('0x1B96B92314C44b159149f7E0303511fB2Fc4774f').call();
        let bnbPrice =  busdBalance / bnbBalance;
        // get kpop price in bnb:
        let mytoken = new web3.eth.Contract(window.erc20_abi, '0x3Ba2b1C2c46200e826C56550ff7a2b29bad10F3d');
        let bnbPoolBalance = await wbnb.methods.balanceOf('0x83ca76bdc2e454e362826c25b8f4abd0791bb594').call();
        let tokenPoolBalance = await mytoken.methods.balanceOf('0x83ca76bdc2e454e362826c25b8f4abd0791bb594').call();
        let tokenBnbPrice = bnbPoolBalance / tokenPoolBalance;
        window.kpopUsdPrice = bnbPrice*tokenBnbPrice;

        // Calculate Lp token value:
        let lpToken = new web3.eth.Contract(window.erc20_abi, '0x83Ca76Bdc2e454E362826c25b8F4Abd0791Bb594');
        ...
        let myContract = new web3.eth.Contract(window.farming_abi, window.farming_address);
...
class ConnectYourWallet extends Component {
  render() { 
    return (
      <div style={{textAlign:'center', color:'white', fontSize:'2em'}}>
